(function () {
  ...
  Object.assign(translations, {
    'The Creator 2.0 package captures modular building blocks, not just a single finished reel. That makes speed and isolation immediately possible downstream without having to wait 3 more weeks for new variations.': 'Creator 2.0 素材包捕捉的是模块化素材组件，而不只是一个完成版短片。这样下游可以立即实现快速生产和变量拆分，不必再等待 3 周来制作新变体。',
    'The Content Manager defines the format and required building blocks. The request then follows one of four operating lanes based on the capability required.': '内容经理定义形式和所需素材组件。请求根据所需能力进入四条工作流之一。',
    'Route visual support requests that make the message, mechanism, or proof easier to understand. Static display ads follow a separate workflow.': '将能让信息、机制或证据更易理解的视觉支持请求路由给设计师。静态展示广告遵循独立工作流。',
    'Mechanism diagrams': '机制图', 'Stat cards': '数据卡', 'Product B-roll overlays': '产品 B-roll 叠加', 'Proof statements': '证据陈述',
    'Graphic Designer': '平面设计师', 'CM defines the claim, visual job, dimensions, and placement': 'CM 定义主张、视觉任务、尺寸和放置位置', 'Jimmy confirms approved RTB and required evidence': 'Jimmy 确认已批准的 RTB 和所需证据', 'Graphic Designer creates the element and source files': '平面设计师制作图形元素和源文件', 'Editor applies the approved element across the test variations': '剪辑师将批准的元素应用到各个测试变体中',
    'Science authority scripts': '科学权威脚本', 'Medical explanation': '医学解释', 'SAB quotes': 'SAB 引用', 'SAB testimonials': 'SAB 证言', 'Request Taylor support when the script or asset needs scientific authority, expert language, or a credible SAB testimonial.': '当脚本或素材需要科学权威、专家语言或可信的 SAB 证言时，请求 Taylor 支持。', 'CM + Jimmy submit the angle, RTB, and requested deliverable': 'CM + Jimmy 提交信息角度、RTB 和所需交付物', 'Taylor supports compliant science-authority scripting and terminology': 'Taylor 支持合规的科学权威脚本和术语', 'Taylor / SAB provides approved quotes or testimonials when available': 'Taylor / SAB 在可行时提供批准的引用或证言', 'CM logs attribution, approval, and usage requirements before production': 'CM 在制作前记录署名、批准和使用要求',
    'SOURCE BREAKDOWN': '来源拆解', 'Content Manager competitor analysis': '内容经理竞品分析'
  });
  translations['After aligning with the media buyer, the Content Manager defines the format and required assets for the following 2 weeks.'] = '与媒体买手对齐后，内容经理定义未来两周所需的形式和资产。';
